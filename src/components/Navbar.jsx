...
const Navbar = () => {
  ...
  return (
    <div>
        <div className={`w-full h-[80px] px-2 smallest:px-4 mobile:px-8 laptop:px-20 flex items-center fixed z-20 top-0 ${scrollPosition > 0 ? 'bg-white shadow' : 'bg-white shadow'} `}>
            <div className='flex items-center justify-between w-full'>
                <div className='smallest:w-[150px] w-[100px] tablet:w-[180px]'>
                    {scrollPosition > 0 ? <Link to ="/"><img src={Logo}/></Link> : <Link to ="/"><img src={Logo}/></Link>}
                </div>
                <div>
                    <ul className = "desktop:flex items-center gap-10 hidden text-black">
                        <li className={`text-xl ${scrollPosition > 0 ? 'text-black' : 'text-black'} cursor-pointer`}><NavLink to = "/">Home</NavLink></li>
                        <li className={`text-xl ${scrollPosition > 0 ? 'text-black' : 'text-black'} cursor-pointer`}><NavLink to = "/tech">DeepTech</NavLink></li>
                        <li className={`text-xl ${scrollPosition > 0 ? 'text-black' : 'text-black'} border-transparent transition-border ease duration-300 cursor-pointer`}><NavLink to = "/homes">DeepHomes</NavLink></li>
                        <li className={`text-xl ${scrollPosition > 0 ? 'text-black' : 'text-black'} border-transparent transition-border ease duration-300 cursor-pointer`}><NavLink to = "/farms" >DeepFarm</NavLink></li>
                    </ul>
                </div>
                <div>
                    <Link className={`text-xl border px-9 py-2 rounded-[7px] ${scrollPosition > 0 ? 'border-[#006837] text-[#006837]' : 'border-[#006837] text-[#006837]'} hidden smallest:hidden desktop:block`} to = "/contact-us">Contact Us</Link>
                </div>
                <p className ="desktop:hidden user-select-none">
                    {!isMenuClicked &&
                    <span className={`material-symbols-outlined cursor-pointer text-3xl ${scrollPosition > 0 ? 'text-black':'text-black'} `} onClick ={handleMenuOpen}>
                        menu
                    </span>
                    }
                </p>
